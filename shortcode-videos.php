...
function gkt_video_sc_create( $atts ) {
	...
	if ( ! empty( $gkt_video_sc_atts['youtube-url'] ) ) {
		...
		if ( ! empty( $matches[1] ) ) {
			...
			$script = '<script>gktviLoadVideo("' . $gkt_yt_video_id . '", "' . $gkt_video_sc_thumbsrc . '", "' . $gkt_video_sc_atts['class'] . '", "' . $gkt_video_sc_url . '", ' . $gkt_video_sc_atts['width'] . ', ' . $gkt_video_sc_atts['height'] . ');</script>';
